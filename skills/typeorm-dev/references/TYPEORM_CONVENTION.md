...
- [SHOULD] Exclude sensitive columns (e.g., passwords) from default queries using `select: false`.
...
- [MUST] Apply `DecimalTransformer` to monetary (decimal) columns. Caution: without it, `number` operations will behave as string concatenation, causing bugs.
...
- [MUST] Use the `Relation<>` wrapper for relation types. Caution: direct reference like `user: User` risks circular dependencies.
- [MUST] Explicitly define FK columns in `@ManyToOne` (enables direct FK access when relation is not loaded).
...
- [MUST] Explicitly specify the join table name and column names in `@JoinTable()` for `@ManyToMany`.
...
- [MUST] Parameter binding: use `:paramName` syntax. Do not use string interpolation (SQL Injection risk).
- [SHOULD] Select only necessary columns. Use `leftJoinAndSelect` when relation data is needed; use `leftJoin` when only conditions are needed.
...
- [MUST] Manage transactions at the Service layer. Do not initiate them from Controller/Repository.
...
- [MUST] Default relation loading: keep lazy (default). Explicitly load with `relations` option when needed.
- [MUST NOT] Do not load relations individually inside loops (N+1 problem). Use `relations` option or `leftJoinAndSelect`.
- [SHOULD] Use `select` option to query only necessary columns.
...
- [MUST NOT] Do not use string interpolation in QueryBuilder (SQL Injection risk).
- [MUST NOT] Do not reference Entities directly without the `Relation<>` wrapper (circular dependency risk).