...
> Default rules for using DECIMAL follow DATABASE_CONVENTION.md. Below is a MySQL-specific precision guide.

### DECIMAL Precision

- **Rule**: [SHOULD] The default precision for amount fields should be `DECIMAL(15, 2)`. Use `DECIMAL(15, 4)` when GAAP compliance is required.
- **Good Examples**:
  ```sql
...
- **Rule**: [SHOULD] Use JOINs instead of subqueries when possible. The MySQL optimizer generally optimizes JOINs more efficiently.
...
| INT | 4 bytes | 0 ~ approx. 4.2 billion | General-purpose integers |
...
- **Rule**: [MAY] JSON type may be used for unstructured data with a flexible structure or frequently changing schema.
- **Rule**: [MUST NOT] Do not store structured data that can be normalized in JSON. (Fields inside JSON are difficult to index directly and cannot be joined)
- **Good Example**:
  ```sql
...
- **Rule**: [SHOULD] Choose a partitioning key from columns frequently used in query WHERE conditions.
- **Good Example**:
  ```sql
  CREATE TABLE log (
      id CHAR(36) NOT NULL,
...
- **Rule**: [SHOULD] Set the slow query threshold to 1 second. (May be adjusted based on project requirements)
...
| Very large rows value | Scanning a large number of rows | Improve indexes or query structure |
...
### Bulk Data Processing in a Single Transaction
