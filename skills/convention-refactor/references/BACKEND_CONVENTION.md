...
> For rules specific to a particular framework, refer to the documents in subdirectories.
>
...
> API design principles, request/response format, HTTP status codes, versioning, filtering/sorting, bulk operations, asynchronous processing, file upload, idempotency, caching, Rate Limiting, OpenAPI standards, etc.
...
> For extended DTO patterns (pagination response, error response, bulk request, etc.), refer to the [API Spec Convention](api-spec/API_SPEC_CONVENTION.md).
...
### Excessive Branching Within Endpoints

- **Rule**: [SHOULD NOT] A single API endpoint should not perform completely different logic based on query parameters.

### Swallowing Errors

- **Rule**: [MUST NOT] Do not catch exceptions and silently ignore them without any handling.
> Empty catch blocks make it impossible to identify the root cause of issues.