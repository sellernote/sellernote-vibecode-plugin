...
> This document defines common rules that apply across all databases.
...
- **Rule**: [MAY] Intentional denormalization is allowed when read performance is critical. However, the strategy for maintaining data consistency during denormalization must be documented.

### ERD Guidelines

- **Rule**: [MUST] Major domain entities and relationships must be documented with an ERD.
...
- **Rule**: [MUST] FK columns must follow the `{referenced_table_singular}_id` pattern.
- **Good Examples**:
  ```sql
  -- user 테이블의 PK
...
- **Rule**: [SHOULD] Tables that require deletion history for business purposes should implement soft delete using the `deleted_at` field.
...
- **Rule**: [MUST] All tables must include a `_no` column (BIGINT AUTO_INCREMENT) with a UNIQUE KEY index.
- **Rule**: [MUST NOT] Do not expose `_no` values in external API responses. Always use `id` (UUID) for external communication.

## Data Types

### Type Selection Criteria

- **Rule**: [MUST] Choose the smallest type appropriate for the data being stored.
...
- **Rule**: [MUST] Store date/time data in UTC and convert the timezone at display time.
...
- **Rule**: [MUST] Schema changes must be managed through migration files. Do not execute DDL directly on the database manually.
- **Rule**: [MUST] Migration file names must use timestamp-based ordering to guarantee sequence.
- **Good Examples**:
  ```
...
- **Rule**: [MUST] Schema changes in production must be performed without service interruption.
